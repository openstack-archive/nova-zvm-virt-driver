...
"""),
    ...
    cfg.StrOpt('zvm_scsi_pool',
               default='xcatzfcp',
               help="""
The name of xCAT SCSI pool.

It's a pool name that zvm openstack solution will use to create a pool
file which maintains the volume carved and allocated to deployed
servers.

Possible values:
    A string which contains a valid file name.
"""),
...
range of addresses connected with a hyphen. Multiple values
are specified with a semicolon connecting them.

Required only if persistent disks are to be attached to virtual server
instances. Each instance needs one FCP in order to attach a volume to itself.
