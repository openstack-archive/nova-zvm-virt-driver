...
This is a z/VM userid which is allowed to logon using the BY keyword
to the guests created by z/VM openstack solution, compatible with the LOGONBY
keyword of the user directory statement.  This value is only used when a guest
is created. If you change this value, existing guests' directory entries are
not automatically updated with the new value.
...
Usage note:
    ...
    non-empty string is provided, you can use LOGONBY xxx command to log on the
