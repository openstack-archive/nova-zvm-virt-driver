...
"""),
    cfg.StrOpt('zvm_default_admin_userid',
               default='',
               help="""
Default log on by user id for the cloud.

Blank-delimited list of z/VM userid allowed to logon using the BY keyword
to the guests created by zVM, compatible with the LOGONBY keyword of the
user directory statement.  This value is only used when a guest is created.
If you change this value existing guests' directory entries are not
...
Usage note:
    When this value is '' which is empty, you can't log on your cloud created
    instance through 3270 protocol; otherwise, you can use
    LOGONBY xxx command to log on the user with its admin userid's password.
    see z/VM administrator guide for additional information.
