...
This is a set of z/VM userid which are allowed to logon using the LOGONBY
keyword to the guests created by z/VM openstack solution, compatible with the
LOGONBY keyword of the user directory statement. This value is only used when a
guest is created. If you change this value, existing guests' directory entries
are not automatically updated with the new value.
...
Usage note:
    ...
    you can use any one of 'oper1', 'oper2', 'oper3', 'jones' as admin user.

    see z/VM CP Planning and Administration for additional information.

Possible values:
    A string with blank as delimiter, with maximum 8 separated strings.
