...
def punch_iucv_file(os_ver, zhcp, zhcp_userid, instance_name):
    ...
    if((distro == "rhel" and release < '7') or (
        ...
        if(distro == "rhel" and release >= '7'):
            iucv_service_path = '/lib/systemd/system/iucvserd.service'
        if(distro == "sles" and release >= '12'):
            iucv_service_path = '/usr/lib/systemd/system/iucvserd.service'
    ...
    cmd = '\n'.join((
        ...
        "cp -rf %s/iucvserv %s 2>&1 >/var/log/messages" % (punch_path,
                                                           iucv_server_path),
        "cp -rf %s/iucvserd.service %s 2>&1 >/var/log/messages" % (punch_path,
                                                        iucv_service_path),
