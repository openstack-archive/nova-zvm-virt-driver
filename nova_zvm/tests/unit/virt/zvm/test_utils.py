...
from nova_zvm.virt.zvm import conf as conf
...
class TestZVMUtils(test.NoDBTestCase):
    ...
    def test_connector_request_handler_invalid_url(self):
        ...
        self.assertRaises(exception.NovaException, rh.call, 'guest_list')
    ...
    def test_connector_request_handler_error(self, mock_send):
        ...
        rh = zvmutils.ConnectorClient(self._url)
        exc = self.assertRaises(exception.NovaException, rh.call,
                                'guest_list')
        self.assertEqual('zVM Cloud Connector request failed',
                         exc.format_message())
        self.assertEqual(expected, exc.kwargs['results'])

    @mock.patch('nova_zvm.virt.zvm.utils._get_instances_path')
    ...
    @mock.patch('nova_zvm.virt.zvm.utils._create_config_drive')
    @mock.patch('nova_zvm.virt.zvm.utils.get_instance_path')
    ...
    @mock.patch('nova_zvm.virt.zvm.configdrive.ZVMConfigDriveBuilder.make_drive')
