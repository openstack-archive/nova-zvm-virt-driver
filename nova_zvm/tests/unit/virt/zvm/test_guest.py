...
class TestZVMGuestOp(test.NoDBTestCase):
    ...
    @mock.patch('nova_zvm.virt.zvm.utils.ConnectorClient.call')
    @mock.patch('nova_zvm.virt.zvm.guest.Guest._import_spawn_image')
    ...
    @mock.patch('nova_zvm.virt.zvm.guest.Guest._setup_network')
    @mock.patch('nova_zvm.virt.zvm.guest.Guest._set_disk_list')
    @mock.patch('nova_zvm.virt.zvm.utils.generate_configdrive')
    @mock.patch('nova_zvm.virt.zvm.guest.Guest._get_image_info')
    @mock.patch('nova_zvm.virt.zvm.utils.ConnectorClient.call')
    ...
    def test_snapshot_capture_fail(self, mock_capture, get_image_service):
        ...
        image_id = 'e9ee1562-3ea1-4cb1-9f4c-f2033000eab1'
        get_image_service.return_value = (image_service, image_id)
        mock_capture.side_effect = exception.NovaException(error='error')

        self.assertRaises(exception.NovaException, self._guest.snapshot,
    ...
    def test_snapshot_import_fail(self, mock_import, mock_delete,
        ...
        mock_import.side_effect = exception.NovaException(error='error')
